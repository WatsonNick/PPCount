...
while (camera.isOpened()):
    ...
    for (xA, yA, xB, yB) in pick:
        ...
        if yA < 0 or xA < 0 or yB > frame.shape[0] or xB > frame.shape[1]:
            continue  # Skip this rectangle if it's outside the frame
        rectf=frame[yA:yA+yB,xA:xA+xB]
        try:
            rectf_resized = cv2.resize(rectf, (64, 128))  # Resize the image to a common size
            imageset.append(rectf_resized)
        except cv2.error as e:
            print(f"Error resizing image: {e}")
            continue
        imageset2=np.asarray(imageset, dtype='object')
        xc=(xA + xB) /2
        yc=(yA + yB) /2
